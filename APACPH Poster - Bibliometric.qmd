...
```{r}
subset1 <- tibble(Article = rownames(bibds_pm), Year = bibds_pm$PY) %>%
  filter(Year <= 2012) %>% 
  group_by(Year) %>% 
  summarise(n = n())
subset2 <- tibble(Article = rownames(bibds_pm), Year = bibds_pm$PY) %>%
  filter(Year >= 2010) %>% 
  group_by(Year) %>% 
  summarise(n = n())
...
tibble(Article = rownames(bibds_pm),
  ...
  geom_point() +
  geom_line() +
  geom_smooth(data = subset1, se = F, method = "lm", aes(y = n+2), linetype = 2) +
  geom_smooth(data = subset2, se = F, method = "lm", aes(y = n+4), linetype = 2) +
